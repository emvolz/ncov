...
> pb
                           pseudo ML        2.5 %       97.5 %
Time of common ancestor 2.019921e+03 2.019800e+03 2.019947e+03
Mean substitution rate  7.923358e-04 4.733357e-04 1.326323e-03

 For more detailed output, $trees provides a list of each fit to each simulation 
> 
> print( date_decimal( td$timeOf ))
[1] "2019-12-03 06:35:49 UTC"
> print( date_decimal( pb$timeOf ))
                     2.5%                     97.5% 
"2019-10-19 22:52:39 UTC" "2019-12-12 18:48:16 UTC" 
...
if (TRUE)
{
	png( 'treedaterTimeTree-nextstrain02feb2020.png', width = 480*3, height = 480*2) 
	par( mfrow = c( 1,2 ))
	...
	png('treedaterTMRCA-nextstrain02feb2020.png' , width = 480, height = .75*480 )
...
png( 'treedater-rtt-nextstrain02feb2020.png', width = 2* 480, height = 1.7 * 480, pointsize=24) 
rootToTipRegressionPlot( td , show.tip.labels=F, pch = 20, cex = 1,bty='n' ) 
dev.off()
invisible('
Root-to-tip mean rate: 0.00076259921361975 
Root-to-tip p value: 0.00582833594938818 
Root-to-tip R squared (variance explained): 0.192719035927523 
...
of <- function(theta, t0 = td$timeOf, bdt = bdt0 ){
	...
	.x0 <- x0  
	.x0['i'] <- unname(i0)
	rv = colik( bdt, .theta, model, x0 = .x0, t0 = t0, res = 100 , forgiveAgtY = TRUE )
	print( c( r, i0, rv ))
	rv
}
#~ of( c( log(60), log(1) ) )

#~ f0 <- optim( par = log(60), fn = of, control = list(fnscale=-1) )
#~ f0 = optimize( of, lower = log(1), upper = log(100), maximum = TRUE )
...
png('phydynRDoublingTimes-a3-nextstrain02feb2020.png' , width = 480, height = .75*480 )
